...
defineFeature(feature, test => {
  test('By default, when user hasn\'t searched for a city, show upcoming events based on the user\'s location', ({ given, when, then }) => {
    given('user hasn\'t searched for any city', () => {
      //nothing has happened so can be left blank
    });
  ...
  test('User can select a city from the suggested list', ({ given, and, when, then }) => {
    let AppWrapper;
    given('user was typing "Arvada" in the city textbox', () => {
      AppWrapper = mount(<App />);
      AppWrapper.find('.city').simulate('change', { target: { value: 'Arvada' } });
    });
    ...
    when('the user selects a city (e.g., "Arvada, Colorado, USA") from the list', () => {
      AppWrapper.find('.suggestions li').at(0).simulate('click');
    });

    then('their city should be changed to that city (i.e., "Arvada, Colorado, USA")', () => {
